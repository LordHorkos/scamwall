...
#
# Extracted to scripts/lib/docker-resources.sh so the operator handoff script
# uses THIS implementation rather than growing a second, weaker one. The rules
# and the reasons behind them live in that file; nothing about the behaviour
# here changed when it moved.
RESOURCE_LIB="$SCRIPT_DIR/lib/docker-resources.sh"
if [ ! -f "$RESOURCE_LIB" ]; then
  printf 'fatal: resource-tracking library not found: %s\n' "$RESOURCE_LIB" >&2
  printf 'fatal: refusing to create Docker resources with no cleanup implementation\n' >&2
  exit 2
fi
# shellcheck source-path=SCRIPTDIR
# shellcheck source=lib/docker-resources.sh
. "$RESOURCE_LIB" || {
  printf 'fatal: the resource-tracking library could not be sourced: %s\n' "$RESOURCE_LIB" >&2
  exit 2; }
